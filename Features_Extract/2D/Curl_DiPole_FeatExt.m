function [DiPoleFeatures] = Curl_DiPole_FeatExt(Curlz,numChannels)

M=sqrt(numChannels);
N=M;

PositiveValue=max(max(Curlz));
NegativeValue=min(min(Curlz));
[x1,y1]=find(Curlz==PositiveValue);
[x2,y2]=find(Curlz==NegativeValue);
scaleFactor=255/(N-1);
